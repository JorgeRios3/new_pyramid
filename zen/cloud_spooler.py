...
import http.client  # provides some high level HTTP services
...
import urllib.request, urllib.parse, urllib.error  # high level interface for www data using URLs
import urllib.request, urllib.error, urllib.parse  # adds functionality for complex URL interaction
...
CRLF = "\r\n"
BOUNDARY = "jaja"  # mimetools.choose_boundary()
...
def oauth_uri_for_code():
    ...
    try:
        d = urllib.parse.urlencode(datos)
...
def oauthlogin():
    ...
    try:
        ...
        chorizo = "{}?{}".format(url, d)
        print(("lo que hay que llamar ", chorizo))
        stream = urllib.request.urlopen(url, d).read()
        print(stream)
        result = True
    except:
        print_exc()
...
class CloudSpooler(object):
    ...
    def oauthlogin(self):
        ...
        datos = dict(
            client_id=CLIENT_ID,
            redirect_uri="https://zen.grupoiclar.com/api/oauth",
            response_type="code",
            scope="https://www.googleapis.com/auth/cloudprint",
        )
        ver = requests.get(url, data=datos).json()
    ...
    def gaiaLogin(self):
        ...
        form = (
            "ltmpl=login&fpui=1&rm=hide&hl=en-US&alwf=true"
        ...
        login = http.client.HTTPS(self.GAIA_HOST, 443)
    ...
    def refreshAuthTokens1(self):
        ...
        value = "https://www.google.com/cloudprint/getauthcode?printerid={}&oauth_client_id={}".format(
            printer, CLIENT_ID
        )
    ...
    def refreshAuthTokens(self):
        ...
        stream = urllib.request.urlopen(self.LOGIN_URL, urllib.parse.urlencode(params))
        print("stream")
    ...
    def getPrinterStatus(self, idPrinter):
        ...
        try:
            ...
            response = self.submit(
                "%s/printer?printerid=%s&extra_fields=connectionStatus"
                % (self.CLOUDPRINT_URL, idPrinter),
                token,
            )
    ...
    def getPrinters(self, proxy=None):
        ...
        if proxy:
            response = self.getUrl(
                "%s/list?proxy=%s" % (self.CLOUDPRINT_URL, proxy), self.tokens
            )
        else:
    ...
    def submitPdf(self, printerid, jobsrc):
        ...
        response = self.getUrl(
            "%s/submit" % self.CLOUDPRINT_URL, self.tokens, data=edata, cookies=False
        )
    ...
    def getUrl(self, url, tokens, data=None, cookies=False, anonymous=False):
        ...
        """
        print(
            "tokens, cookies, anonymous, url, data",
            tokens,
            cookies,
            anonymous,
            url,
        ...
        if not anonymous:
            if cookies:
                ...
                request.add_header(
                    "Authorization", "GoogleLogin auth=%s" % tokens["Auth"]
                )
                # esto cambia
