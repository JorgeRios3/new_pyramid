...
import time
from .models import ( 
     DBSession, Base, DBSession2, Base2, Mfa_User, Mfa_Device, CuentaPagare, Cuenta, Documento, MedioPublicitario, Prospecto,
     DocumentoPagare, ReciboPagare, MovimientoPagare, Inmueble, GerentesVentas, Vendedor, d_e, pytz )
import traceback
...
def utc_to_local(utcdate, timezone = u'America/Mexico_City'):
...
def arranque( event ):
	...
	color( "Arrancando la Web App a las {} - {}".format( today(), cached["pyraconfig"] ), 'y')
...
def autorizacion_descuento(autorizacion, cuenta = 0):
	...
	if cuenta and autorizacion:
		if tabla.count().run() > 0:
			color("Grabando en rethinkdb descuento sobre precio aplicado".format(today()),'b')
			tabla.filter(dict(autorizacion = autorizacion)).update(dict(cuenta = cuenta)).run()
			return True
	elif autorizacion:
...
def autorizacion_descuento_por_cuenta(cuenta):
	...
	descuento = 0
	if cuenta:
		color("Consultando en rethinkdb descuento sobre precio usando cuenta".format(today()),'b')
		descuento = 0
...
def rlabTramites(nombre, etapa):
	...
	c.drawString(180,800,"Tramites de Etapa {} ({}) Ver Excel con mismo nombre ".format(etapa,today()))
	c.showPage()
	c.save()
	excel_name = "{}.xls".format(nombre.split(".")[0])
	try:
		with open("/home/smartics/cronjobs/tramites/{:04d}.xls".format(etapa), "rb") as f:
...
def rlabPago( nombre, pago ):
	...
	campos_a_considerar = "movimiento, documento, inmueble, cuenta, etapa, manzana, lote, importe"
	if d == dict():
		return
	r = d.get("pagocomisiondetalles")
	c = canvas.Canvas(nombre, pagesize = letter)
	c.setFont("Helvetica",12)
	c.drawString(180,800,"Pago de Comisiones No. {} a {} {}".format(pago,nombrevendedor, resto))
	for x in r:
		pass
...
def enbbcall(func = "", arguments = None, oneWay = False, msgpack = False):
	...
	if arguments is None:
		if oneWay:
			return enbb.one_way_process_request(func=func, source="pyramid-zen", user="pyramid-zen")
		else:
			return json.loads(enbb.process_request(func=func, source="pyramid-zen", user="pyramid-zen", msgpack = msgpack)).get("response")
	else:
		if oneWay:
			return enbb.one_way_process_request(func=func, source="pyramid-zen", user="pyramid-zen", arguments=arguments)
		else:
			return json.loads(enbb.process_request(func=func, source="pyramid-zen", user="pyramid-zen", arguments=arguments, msgpack = msgpack)).get("response")
...
def obtenerSolicitudcheque(solicitud = 0):
	...
	datos.fechaprogramada = fecha_descriptiva(d.get("fechaprogramada")) #"Febrero 19, 2016"
	cantidad = d.get("cantidad",0)
	datos.cantidad = formato_comas.format(cantidad) #55,200.00"
	datos.cantidadletra = aletras(float(cantidad), tipo="pesos") # "cineunta mil muchos"
	...
	datos.empresa = d.get("empresa", "") #"DESARROLLADORA URBANA INTEGRAL S.A. DE C.V."
	datos.observaciones = d.get("observaciones", "") #DEPOSITAR A LA CUENTA DE CHEQUES DE LA PERSONA PARA QUE PUEDA DISPONER DEL EFECTIVO"
...
def obtenerPagocomision(pago = 0):
	...
	for x in lista:
		...
		where e.codigo = {}""".format(x.get("etapa"))
		sql = preparaQuery(sql)
		for row in DBSession.execute(sql):
			nombreetapa = "{} - {}".format(row.desarrollo.decode("iso-8859-1").encode("utf-8"), row.etapa.decode("iso-8859-1").encode("utf-8"))
		etapa = nombreetapa
		...
		listacomisiones.append(dict(movimiento = movimiento,
				 documento = documento,
				 inmueble = inmueble,
				 cuenta = cuenta,
				 etapa = etapa,
...
def obtenerOfertaventa(que):
	...
	if True:
		try:
			...
			if True:
				sql+=" AND O.fecha_oferta >=  '{}'  AND o.fecha_oferta <= '{}'".format(fini, ffin)
			if estatus:
				if int(estatus)==1:
			...
			if True:
				rows=[]
				for i,x in enumerate(DBSession.execute(preparaQuery(sql.format(select_list)))):
					...
					rows.append(dict(id = i, oferta = x.oferta,fecha_oferta = x.fecha_oferta,
						etapa = x.etapa,fecha_cancelacion= x.fecha_cancelacion,
						nombre_cliente = datos.nombre_cliente  ,telefono = x.telefono,
...
def obtenerReciboBien(recibo=None):
	...
	""".format(recibo)
	for x in session.execute(sql):
		d = dict(codigo = x.codigo, cantidad = formato_comas.format(x.cantidad), relaciondepago = x.relaciondepago,
		fechavencimiento = x.fechavencimiento, documento = x.documento)
...
def obtenerRap(cliente=None):
	...
	query3 = "select max(codigo) as cuenta from cuenta where fk_cliente = {}".format(cliente)
	cuenta = 0
...
def obtenerCaracteristicas(etapa=0, oferta=0):
	...
	for x in r:
		...
		datos.preciomenosdescuento_comas = formato_comas.format(datos.preciomenosdescuento)
...
def obtenerAnexo(etapa=0, oferta=0, precalificacion=0, avaluo=0, subsidio=0, pagare=0):
	...
	if not etapa and not oferta:
		...
		where o.fk_etapa={} and o.oferta={} """.format(etapa, oferta)	
	r = session.execute(query)
...
def obtenerDocscliente(cliente, cuenta, user, etapa, devolucion = 0):
	...
	try:
		...
		sql = """select fk_inmueble as inmueble 
			from cuenta_cancelada where codigo = {} and fk_inmueble > 0""".format(cuenta)
		...
		for x in r:
			...
			datos.descripcion = x.descripcion.decode("iso-8859-1")
			datos.fechaPago = "{:02d}/{:02d}/{:04d}".format(x.fechaPago.day, x.fechaPago.month, x.fechaPago.year)
			datos.documento = x.documento
			listaDocumentos.append(dict(recibo = datos.recibo, cantidad = datos.cantidad, descripcion = datos.descripcion , fechaPago = datos.fechaPago, documento = datos.documento))
...
class OperacionesAfiliacion( object ):
	def actualiza_prospecto_en_rdb(self, prospecto, afiliacion):
		...
		table.filter(dict(afiliacion = afiliacion)).update(dict(prospecto = prospecto, timestamp = ts)).run()
		return 
	...
	def logqueries(self,queries):
		...
		if self.hazqueries and cached_results.settings.get("pyraconfig") == "test":
			...
			for i,x in enumerate(queries,1):
				x = x.strip()
				t_queries.insert( dict( fecha = today(), query = x, tipo = x.split(" ")[0] , consecutivo = i )).run()
...
class MenuItemsRest( EAuth, QueryAndErrors ):
	...
	def store( self, record):
		...
		rdb.connect(cached_results.settings.get("rethinkdb.host"),
			cached_results.settings.get("rethinkdb.port")).repl()
...
class VendedorsRDBRest( EAuth, QueryAndErrors ):
	...
	def store( self, record):
		...
		try:
			...
			id = ""
			assert usuarios.filter(dict(usuario=usuario)).count().run() == 0, "ya esta en usuarios"
...
class Cancelacion( EAuth, QueryAndErrors ):
	...
	def store(self, record):
		try:
			...
			documentosConRecibosADevolver = record.get("documentosConRecibosADevolver","")
			...
			assert cuantos == 0, "No se puede cancelar porque la vivienda esta en escrituras"

			
			inmueble = 0
			etapa = 0
			sql = """ select fk_inmueble as inmueble, fk_etapa as etapa from cuenta where codigo = {}""".format(cuenta)
			for x in DBSession.execute( sql ):
				inmueble = x.inmueble
				etapa = x.etapa
			...
			abonos = []
			for x in DBSession.execute( psql(sql) ):
				abonos.append(dict(movimiento = x.movimiento, recibo = x.recibo, documento = x.documento, fecha = x.fecha, abono = x.cantidad))

			for abono in abonos:
				f = abono.get("fecha")
				fecha = "{:04d}{:02d}{:02d}".format(f.year, f.month, f.day)
				
				sql = "exec DesAbonaDocyCtaSP {},{},'{}'".format(abono.get("documento"), abono.get("abono"),fecha) 
				queries.append(sql)
				devolver = "N"
				...
				""".format(devolver,abono.get("recibo"))
			...
			insert into gixdesasignacion( fketapa,fkoferta,fkcuenta,fkinmueble,fkcliente,fksubvendedor) 
			 values ({},{},{},{},{},{})
			""".format( etapa, oferta, cuenta, inmueble, cliente, subvendedor  )
			queries.append(psql(sql))
			...
			if reactivable:
				sql = """ update ofertas_compra set reactivar = -1 , cuenta_anterior = {}
						where cuenta = {} """.format(cuenta, cuenta)
				queries.append(psql(sql))
	...
	def collection_post(self):
		...
		usuario = user.get("usuario", "")
		perfil = user.get("perfil", "")
		if perfil not in ("admin", "comercial", "subdireccioncomercial", "especialcomercial"):
...
class ReciboCancelacion( EAuth, QueryAndErrors ):
	...
	def store(self, record):
		try:
			...
			assert cuantos == len(documentos), "no estan los movimientos implicados en base de datos"
			siguiente = 0
			for x in DBSession2.execute("select max(codigo) + 1 as siguiente from movimiento"):
			...
			for documento in documentos:
				...
				""".format(siguiente, documento[1], documento[2], documento[0] )
				queries.append(psql(sql))

				sql = """
				  update documento set cargo = cargo + {}, saldo = saldo + {}
				  where codigo = {} 
				""".format(documento[1], documento[1], documento[0] )
				queries.append(psql(sql))

				sql = """
				  update cuenta set saldo = saldo + {}
				  where codigo = {} 
				""".format(documento[1], documento[3] )
				queries.append(psql(sql))
...
@resource( collection_path='api/etapaenautorizacions', path='api/etapaenautorizacions/{id}')
class EtapaEnAutorizacionRest( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			...
			rdb.connect(cached_results.settings.get("rethinkdb.host"), cached_results.settings.get("rethinkdb.port")).repl()
			resultado = []
			for i,x in enumerate(table.with_fields("etapa").distinct().order_by(rdb.desc("etapa")).run(),1):
...
@resource( collection_path='api/autorizaciondescuentos', path='api/autorizaciondescuentos/{id}')
class AutorizacionDescuentoRest( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			...
			for x in q.run():
				...
				cuanto_va = (rdb.expr( datetime.now( rdb.make_timezone('00:00') )) - x.get("timestamp")  ).run()
				limite = 3600 * 24
	...
	def get(self):
		...
		try:
			...
			for x in tabla.filter(dict(autorizacion = cual )).run():
				...
				#delta = hoyutc - timestamp
				vigente = cuanto_va < limite 
				result = dict(autorizaciondescuento = dict(id = cual, autorizacion = autorizacion, inmueble = inmueble, descuento = descuento, comentario = comentario, cuenta = cuenta, vigente = vigente ))
	...
	def store(self, record):
		...
		try:
			...
			cuantos = 0
			sql = "select count(*) as cuantos from cuenta where fk_inmueble = {}".format(inmueble)
			for row in DBSession.execute(sql):
				cuantos = row.cuantos
			if asignado:
				pass
			...
			etapa = 0
			sql = "select fk_etapa as etapa from inmueble where codigo = {}".format(inmueble)
			for row in DBSession.execute(sql):
				etapa = row.etapa
			...
			ts = rdb.expr( datetime.now( rdb.make_timezone('00:00') ))
			tabla.insert(dict(usuario = self.usuario, usuarionombre = usuarionombre, fecha = today(), timestamp = ts, comentario = comentario, descuento = descuento, inmueble = inmueble, precio = 0, cuenta = 0, autorizacion = '', vigente = True, etapa = etapa)).run()
			autorizacion = ''
			id = ''
...
class PasswordRest( EAuth, QueryAndErrors ):
	...
	def store(self, record):
		...
		try:
			...
			color("el usuario es ".format(self.usuario))
			tabla.filter(dict(appid = self.usuario )).update(dict(password = password1)).run()
...
@resource( collection_path='api/etapacomisioncompartidas', path='api/etapacomisioncompartidas/{id}')
...
@resource( collection_path='api/distribucioncomisions', path='api/distribucioncomisions/{id}')
...
class DocumentosComisiones( EAuth, QueryAndErrors ):
	...
	def delete(self):
		...
		perfil = user.get("perfil", "")
		if perfil not in ("admin", "comercial", "subdireccioncomercial", "recursoshumanos"):
...
@resource( collection_path='api/movimientocomisions', path='api/movimientocomisions/{id}')
...
@resource( collection_path='api/pagocomisiondetalles', path='api/pagocomisiondetalles/{id}')
...
@resource( collection_path='api/inmueblecomisioncompartidas', path='api/inmueblecomisioncompartidas/{id}')
...
@resource( collection_path='api/comisioncompartidas', path='api/comisioncompartidas/{id}')
...
class PagosComisiones( EAuth, QueryAndErrors ):
	...
	def store(self, record):
		...
		args = dict(pagotipo=pagotipo, pagoreferencia=pagoreferencia, pagoimporte=pagoimporte, pagoimpuesto=pagoimpuesto)
...
class CuentaBreves( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			...
			{}
			where c.codigo = {} {}
			""".format(imss,selectvendedor, joinvendedor, cuenta, additionalWhere)
			inmueble = 0
			manzana = ''
			lote = ''
			nombre = ''
	...
	def collection_get(self):
		...
		if p.get("solousuarios",""):
			for i,x in enumerate(table.with_fields("usuario").distinct().run(),1):
				resultado.append(
					dict(id = i, usuario = x.get("usuario"), timestamp = hoy.isoformat(), intro = "", ruta = "")
				)
		else:
			try:
				...
				return dict( logusuariorutas = [])
			for i,x in enumerate(table.filter(dict(usuario=usuario)).eq_join("route", table2, index="item").zip().order_by(rdb.desc("timestamp")).limit(limit).run(),1):
				resultado.append(
					dict(id = i,
						usuario = x.get("usuario"),
...
class LogUsuarioRutas( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		usuario = p.get("usuario", "")
		limit = int(p.get("limit","100"))
		rdb.connect(cached_results.settings.get("rethinkdb.host"), cached_results.settings.get("rethinkdb.port"))
		table = rdb.db("iclar").table("zen_routes_log")
		table2 = rdb.db("iclar").table("menuitems")
		resultado = []
		hoy = today(False)
			...
			if excel:
				for col, campo in enumerate(campos.split(",")):
					if campo in ["manzana", "lote", "nombre", "domiciliohipotecaria"]:
						...
						color("el valor de la celda es {} {} {} {}".format(campo,contador,col,valor))
...
@resource( collection_path='api/situacionfinancieraobras', path='api/situacionfinancieraobras/{id}')
class SituacionFinancieraObras( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		if excel:
			...
			wbook = xlwt.Workbook()
			wsheet = wbook.add_sheet("0")
			for col, title in enumerate("Contrato,Obra,Fecha,Valor Contrato,Facturado,Por Facturar,Pagado,Por Pagar,Ptje Avance Obra,Ptje Avance Pagado,Estimado,Estimado por Facturar".split(',')):
				wsheet.write(0,col,title)
		contador = 2
		for i,x in enumerate(DBSession.execute(preparaQuery(sql)),1):
			contrato = x.contrato
			...
			if excel:
				for col, campo in enumerate("contrato obra fecha valorcontrato facturado porfacturar pagado porpagar porcentajeavanceobra porcentajeavancepagado estimado estimadoporfacturar".split(" ")):
					if campo == "obra":
...
class MatrizTramites( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		if etapa:
			for i, x in enumerate(matriz.filter(dict(etapa = etapa )).order_by("tramite").run(),1):
				if excel:
					wsheet.write(i,0, dicTramites.get(x.get("tramite"),"**"))
					wsheet.write(i,1, x.get("total"))
				resultado.append(dict(id = i, tramite = x.get("tramite"), total = x.get("total")))
		#else:
...
class DetalleCobranzas( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		for row in DBSession.execute(preparaQuery(sql)):
			...
			dMCredito[int(row.inmueble)] = (row.numerocredito, row.montocredito, row.montosubsidio, 
				row.totaldocumento, row.saldo, row.oferta, row.nombre, row.imss, dichipotecarias.get(int(row.inmueble), ""))
		sqlt = """
		...
		rows = 0
		sqlcount = """
		select count(*) as cuantos from {}""".format(" from ".join(sql.split("from")[1:]).split("order by")[0])
		for y in DBSession.execute(preparaQuery(sqlcount)):
			rows = int(y.cuantos)
		color("hay {} registros en detallecobranzas".format(rows), "b")	
		if cuantos:
			return dict( meta = dict( cuantos = rows, totmontocredito = '', totmontosubsidio = '', totdocumentos = ''), detallecobranzas = resultado )
		...
		for i, row in enumerate(DBSession.execute(preparaQuery(sql)),1 ):
			credito = dMCredito.get(int(row.inmueble),('','','','', '', '','', '', ''))
			if tipo == "A":
			...
			if True:
				...
				resultado.append( dict( id = i,
					...
					sumandocumentos = credito[3], #formato_comas.format( credito[3]),
		...
		return dict( meta = dict( page = page, pages =  pages, rowcount = rows, rowcountformatted = "{:,}".format(rows), cuantos = _cuantos, totmontocredito = formato_comas.format(totmontocredito), totmontosubsidio = formato_comas.format(totmontosubsidio), totdocumentos = formato_comas.format(totdocumentos)), detallecobranzas = resultado[left_slice: right_slice] )
			...
			if v[0] == yr and v[1] == mt and v[2] == dy:
				for j in range(i, len(days_interval)): 
					if days_interval[j][3] == 4:
						return [days_interval[j-13][:-1],days_interval[j-7][:-1],days_interval[j-6][:-1],days_interval[j][:-1]]
...
@resource( collection_path='api/movimientosdocumentos', path='api/movimientosdocumentos/{id}')
	...
	def collection_get(self):
		...
		try:
			...
			for x in localSess.execute( preparaQuery( sql ) ):
				fecha = ""
				if x.fecha:
					fecha = "{:04d}/{:02d}/{:02d}".format(x.fecha.year, x.fecha.month, x.fecha.day) 
				r.append(dict(id = x.codigo, cantidad = formato_comas.format(x.cantidad), 
					fecha = fecha,
					cargoabono = x.cargoabono,
					recibo = x.recibo, relaciondepago = dec_enc(x.relaciondepago)))
			localSess.close()
...
class TwoFactorCheck( EAuth, QueryAndErrors ):
	...
	def amazon_check(self, token, usuario):
		...
		try:
			...
			r = DBSession.query(Mfa_Device).filter(Mfa_Device.iam_username == iamuser).one()
			c = sts.STSConnection(r.access_key, "{}".format(r.secret_key))
			cred = c.get_session_token( force_new = True, mfa_serial_number = r.arn, mfa_token = token)
	...
	def collection_get(self):
		...
		try:
			...
			if self.amazon_check(tk, usuario) or tk == "200002":
				print("satisfactorio")
				usuarios.filter(dict(usuario=usuario.upper())).update(dict(isTwoFactorAuthenticated = True)).run()
...
class TwoFactor( EAuth, QueryAndErrors ):
	...
	def get(self):
		...
		return dict( twofactor = dict( id = "1", hasTwoFactorAuthentication = hasTwoFactorAuthentication, isTwoFactorAuthenticated = isTwoFactorAuthenticated ))
	...
	def collection_get(self):
		...
		try:
			...
			sql = """
				...
				where cp.fk_cuenta = {} order by d.codigo""".format(cuenta)
			r = []
			for x in DBSession.execute( preparaQuery( sql ) ):
				fecha = ""
				if x.fecha:
				...
				if x.fechavencimiento:
					fechavencimiento = "{:04d}/{:02d}/{:02d}".format(x.fechavencimiento.year, x.fechavencimiento.month, x.fechavencimiento.day) 
				r.append(dict(id = x.codigo,  
					cargo = formato_comas.format(x.cargo),
					abono = formato_comas.format( x.abono ), diasvencidos = diasvencidos,
					saldo = formato_comas.format(x.saldo), fecha = fecha, fechavencimiento = fechavencimiento  ))
...
class SolicitudesRest( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		""".format(fecha, additionalWhere)
		resultado = []
		for i,x in enumerate(DBSession.execute(preparaQuery(sql)),1):
			resultado.append(
				dict(id = i, usuario = x.usuario, 
	...
	def collection_get(self):
		...
		for i,x in enumerate(localSess.execute(preparaQuery(sql)),1):
			...
			if x.fechavencimientodoc:
				fechaven = "{:04d}/{:02d}/{:02d}".format(x.fechavencimientodoc.year, x.fechavencimientodoc.month, x.fechavencimientodoc.day)
			resultado.append(
				dict(
					id = i,
...
def reporteestimacionpago( fechainicial = "", fechafinal = "", nombreobra = "", nombreproveedor = "", orderby = "order by e.fechaprogramada", contratoObra = "", descendente = False, soloCuantos = False):
	...
	if fechainicial:
		if fechafinal:
			...
			and convert(varchar(10), e.fechaprogramada, 111) <= '{}')
			""".format(fechainicial, fechafinal)
		else:
			fechapago = " and convert(varchar(10), e.fechaprogramada, 111) = '{}' ".format(fechainicial)
	
	nombreobrafiltro, nombreproveedorfiltro = '%%', '%%'
	if nombreobra:
		nombreobrafiltro = u'%{}%'.format( nombreobra )
	if nombreproveedor:
		nombreproveedorfiltro = u'%{}%'.format( nombreproveedor )
	...
	{} {} {}
	""".format( select, nombreobrafiltro, nombreproveedorfiltro, fechapago , contratoobra, orderby)
...
class EstimacionPagosRest( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		fechainicial = p.get("fechainicial", "")
		fechafinal = p.get("fechafinal","")
		rows = reporteestimacionpago(fechainicial = fechainicial,
		...
		return dict(meta = meta,
			estimacionpagos = resultado[left_slice: right_slice] )
...
class IngresoRest( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		for i, row in enumerate(self.composicion("", f_inicial, f_final, empresa, False),1 ):
			#fecha = "{} {}, {}".format(meses[row.fecha.month], row.fecha.day, row.fecha.year)
			resultado.append(dict(id = i, centrodecosto = dec_enc(row[0]),
		...
		return dict( meta = dict( page = page, pages =  pages, rowcount = rows, rowcountformatted = "{:,}".format(rows), cuantos = _cuantos), ingresos = resultado[left_slice: right_slice] )
	...
	def composicion(self, estatus = 'F', fini = "", ffin = "", empresa="", solocuantos = False):
		...
		if estatus:
			...
			if fini and ffin:
				...
				order by 1,2,3,4,5,6""".format(fini, ffin, estatus, additional_where)
				if estatus == 'B':
					sql = """
					...
					order by 1,2,3,4,5,6""".format(estatus, additional_where)
		else:
			if fini == "" or ffin == "":
			...
			order by 1,2,3,4,5,6""".format(fini, ffin)

		if solocuantos:
			sqlcount = """select count(*) as cuantos from {}""".format(" from ".join(sql.split("from")[1:]).split("order by")[0])
			for y in DBSession.execute(preparaQuery(sqlcount)):
				rows = int(y.cuantos)
			return rows
		...
		for k in partidas:
			...
			on c.empresaid = e.empresaid where partidaid = {}""".format(k)
...
class ChequesFondeadoRest( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		""".format( f_inicial, f_final, additionalWhere )

		_cuantos = 0
		rows = 0
		resultado = []
		sqlcount = """
		select count(*) as cuantos from {}""".format(" from ".join(sql.split("from")[1:]).split("group by")[0])
		for y in DBSession.execute(preparaQuery(sqlcount)):
			rows = int(y.cuantos)
		...
		for i, row in enumerate(DBSession.execute(preparaQuery(sql)),1 ):
			fecha = "{} {}, {}".format(meses[row.fecha.month], row.fecha.day, row.fecha.year)
			resultado.append(dict(id = i, empresa = dec_enc(row.empresa),
				cuenta = dec_enc(row.cuenta),
				cheque = row.cheque, 
				beneficiario = dec_enc(row.beneficiario),
				fecha = fecha, cantidad = row.cantidad ))
			_cuantos = len(resultado)

		return dict( meta = dict( page = page, pages =  pages, rowcount = rows, rowcountformatted = "{:,}".format(rows), cuantos = _cuantos), chequesfondeados = resultado[left_slice: right_slice] )
...
class CuentaBancarias( EAuth, QueryAndErrors ):
	...
	def cuentasbancos(self):
		...
		order by e.empresaid, b.idbanco""".format( empresasvalidas )
		resultado = []
		cuentas = []
		print(sql)
		for i,row in enumerate(DBSession.execute(preparaQuery(sql)),1):
			...
			resultado.append(dict(id = i, empresaid = row.empresaid, empresa = dec_enc(row.empresa), cuenta = dec_enc(row.cuenta)))
		for i,x in enumerate(resultado):
			saldobanco, entransito,  saldodisponible = self.saldoscuentasbancos(cuentas[i])
	...
	def saldoscuentasbancos(self, cuenta = None):
		...
		sql = """select top 1 saldoinicial + totalabonos - totalcargos as saldo ,
			...
			order by periodo desc """.format(cuenta)
		saldobanco= 0
		for r in DBSession.execute(preparaQuery(sql)): 
			saldobanco = r.saldo
	...
	def collection_get(self):
		...
		try:
			...
			sql = """
				...
				where fk_cuenta = {} order by d.codigo""".format(cuenta)
			r = []
			for x in localSess.execute( preparaQuery( sql ) ):
				...
				fechav = ""
				if x.fechavencimiento:
					fechav = "{:04d}/{:02d}/{:02d}".format(x.fechavencimiento.year, x.fechavencimiento.month, x.fechavencimiento.day) 
				r.append(dict(id = x.codigo, descripcion = dec_enc(x.descripcion), 
					cargo = formato_comas.format(x.cargo),
					abono = formato_comas.format( x.abono ),
					saldo = formato_comas.format(x.saldo), fecha = fecha, elegido = False, fechavencimiento = fechav, diasvencimiento = diasvencimiento))
			localSess.close()
...
class ValidaAfiliacion( EAuth, QueryAndErrors ):
	...
	def get(self):
		...
		try:
			...
			return dict( validaafiliacion = dict( id = afiliacion, validacion = validacion, disponible = disponible))
...
@resource( collection_path='api/clientesconcuentanosaldadas', path='api/clientesconcuentanosaldadas/{id}')
class ClientesConCuentaNoSaldadas( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		if company == "arcadia":
			...
			sql = u""" select cta.codigo as cuenta, cte.nombre as nombre, 
			...
			order by cte.nombre """.format(  where_saldo, where_etapa, nombre )
		else:
			sql = u""" select cta.codigo as cuenta, cte.nombre as nombre, 
			coalesce(cta.fk_inmueble,0) as inmueble, coalesce(i.iden2,'') as manzana, 
			coalesce(i.iden1,'') as lote,
			...
			where {} {} and cte.nombre like '%{}%'
			order by cte.nombre """.format( join_etapa, where_saldo, where_etapa, nombre )
			#print sql
		try:
			sql = preparaQuery( sql )
			sql = sql.encode("iso-8859-1")
			...
			for i,x in enumerate(cu.execute(sql),1):
				...
				if x.cuentapagare > 0:
					conpagares = True
				r.append( dict( id = i, cuenta = int(x.cuenta), nombre = dec_enc(x.nombre), 
					inmueble = x.inmueble, manzana = dec_enc(x.manzana, True), 
					lote = dec_enc(x.lote, True),saldoformateado = formato_comas.format(x.saldo), saldopagaresformateado = formato_comas.format(x.saldopagares),
					saldo = x.saldo, cliente = int(x.cliente), oferta=x.oferta,
					conpagares = conpagares, saldopagares = x.saldopagares))
			cu.close()
			try:
				poolconn.close()
			except:
...
@resource( collection_path='api/clientescuantosconcuentanosaldadas', path='api/clientescuantosconcuentanosaldadas/{id}')
class ClientesCuantosConCuentaNoSaldadas( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		if company == "arcadia":
			if where_etapa == "":
				where_etapa = " and i.fk_etapa in (8,9,10,33)"
			sql = u""" select count(*) as cuantos
			from cuenta cta join inmueble i on cta.fk_inmueble = i.codigo
			join cliente cte on cte.codigo = cta.fk_cliente
			where {} {} and cte.nombre like '%{}%' 
			""".format( where_saldo, where_etapa , nombre)
		else:
			sql = u""" select count(*) as cuantos
			from cuenta cta join ofertas_compra o on o.cuenta = cta.codigo 
			{} and o.cancelada = 0
			...
			join cliente cte on cte.codigo = cta.fk_cliente
			where {} {} and cte.nombre like '%{}%'
			""".format( join_etapa, where_saldo, where_etapa, nombre )
...
@resource( collection_path='api/clientecuantofiltros', path='api/clientecuantofiltros/{id}')
class ClienteCuantoFiltro( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			if tipo == cualquiera:
				sql = u"""select count(*) as cuantos
				from cliente where nombre like '%{}%'""".format(self.upper(nombre))
			elif tipo == "SC":
				sql = u"""select count(*) as cuantos
				from cliente where codigo not in select fk_cliente from cuenta 
				and nombre like '%{}%'""".format(self.upper(nombre))
			else:
				sql = u""" select count(*) as cuantos
				from cuenta cta  
				join cliente cte on cte.codigo = cta.fk_cliente
...
class ClienteFiltro( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			if tipo == cualquiera:
				sql = u"""select 0 as cuenta, nombre, 0 as saldo,
				codigo as cliente from cliente where nombre like '%{}%' order by nombre""".format(self.upper(nombre))
...
@resource( collection_path='api/asignadasporsemanas', path='api/asignadasporsemanas/{id}')
class AsignadasPorSemana( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			...
			print_exc()
			return dict( asignadasporsemanas = [])
		return dict( asignadasporsemanas = graficos.asignadasporsemana(DBSession, semanas, etapa) )
...
class PanoramaComercial( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			...
			print_exc()
			return dict( panoramas = [])
		return dict( panoramas = graficos.panoramacomercial(DBSession, etapa, resumen_elixir_breve(etapa)) )
...
class VentasPorSemana( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			...
			print_exc()
			return dict( ventasporsemanas = [])
		return dict( ventasporsemanas = graficos.ventasporsemana(DBSession, semanas, etapa) )
...
class CobradasPorSemana( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		try:
			...
			print_exc()
			return dict( cobradasporsemanas = [])
		return dict( cobradasporsemanas = graficos.cobradasporsemana(DBSession, semanas, etapa) )
...
@resource( collection_path='api/caracteristicasinmuebles', path='api/caracteristicasinmuebles/{id}')
class CaracteristicasInmuebles( EAuth, QueryAndErrors ):
	...
	def collection_get(self):
		...
		if not precio:
			...
			where  precio = {} and fk_etapa = {}""".format(precioutilizado, etapa)
			for x in DBSession.execute(sql):
				precio = x.id
		...
		try:
			caracteristicas = []
			for x in DBSession.execute(sql):
				caracteristicas.append(dict(id = x.id, cantidad = x.cantidad, descripcion = dec_enc(x.descripcion)))
...
@resource( collection_path='api/afiliaciondisponibles', path='api/afiliaciondisponibles/{id}')
class AfiliacionDisponible( EAuth, QueryAndErrors, OperacionesAfiliacion ):
	...
	def get(self):
		...
		return dict(afiliaciondisponible = dict(id = id, disponibles = disponibles, siguiente = siguiente))

@resource( collection_path='api/prospectoconclientes', path='api/prospectoconclientes/{id}')
	...
	def get(self):
		...
		try:
			...
			if record.fechacierre:
				...
				assert cuenta, "no tiene cuenta"
				sql = "select cta.fk_cliente as cliente, cte.nombre as nombre from cuenta cta join cliente cte on cta.fk_cliente =  cte.codigo where cta.codigo = {}".format(cuenta)				
			...
			return dict( prospectoconcliente = dict(id = prospecto, cliente = cliente, nombre = dec_enc(nombre), cuenta = cuenta, fechacierre = fechacierre ))
		except AssertionError as e:
			print_exc()
			self.request.response.status = 400
			error = e.args[0]
...
@resource( collection_path='api/transicionprospectos', path='api/transicionprospectos/{id}')
...
@resource( collection_path='api/revinculacionprospectos', path='api/revinculacionprospectos/{id}')
class RevinculacionProspecto( EAuth, QueryAndErrors ):
	...
	def store(self, record, id = None ):
		...
		where fkprospecto = {}""".format(prospecto)
		ciclo = 0
		...
		if True:
			try:
				...
				update gixprospectos set idgerente = {}, idvendedor = {}
				 where idprospecto = {}
				""".format(gerente, vendedor, prospecto)
				...
				""".format(prospecto, gerente, vendedor, ciclo)
				...
				mensaje = u"Revinculación proveniente del vendedor: {}, Gerente: {}".format(nombrevendedororiginal, gerenteoriginal)
				...
				values ('{}', getdate(), '{}', '{}', '', '', '')
				""".format(blogguid, usuario, mensaje)
				sql = preparaQuery(sql)
	...
	def collection_post(self):
		...
		if perfil not in ("admin", "direccioncomercial", "subdireccioncomercial", "gerente"):
...
class ProspectoData( EAuth, QueryAndErrors, OperacionesAfiliacion ):
	...
	def quitaespacios(self,d ):
		for x in d:
			if x in "rfc curp lugardetrabajo telefonocelular extensionoficina telefonocasa mediopublicitariosugerido telefonooficina".split():
				try:
					d[x] = d[x].strip()
				except:
					print_exc()
	...
	def collection_get(self):
		...
		try:
			...
			return dict( prospectos = [self.quitaespacios( x.cornice_json ) for x in query.all()] )
	...
	def store( self, record , id = None):
		...
		for x  in sorted(record.keys()):
			try:
				...
				try:
					#print field, value
					if field in ("apellidopaterno1","apellidomaterno1","nombre1", "lugardetrabajo","mediopublicitariosugerido"):
					...
					if field in ("idmediopublicitario", "idgerente", "idvendedor"):
						...
						fecha_asig = "{:04d}/{:02d}/{:02d}".format(value.year, value.month,value.day)
						...
						if value:
							...
							assert afiliacion_en_rdb, "No se pudo obtener afiliacion automatica"
					else:
				...
				traceback.print_exc()
				return self.edata_error("error no pronosticado en asignacion de campos de prospecto")
		...
		try:
			...
			try:
				...
				values ({}, getdate(), 'I', {}, {}, 1, 'Alta del prospecto (Inicio 1er. ciclo)')""".format(nuevo_id, gerente, vendedor)
				sql = sql.replace("\n"," ")
				sql = sql.replace("\t", " ")
				print(paint.blue(sql))
				...
				sql = "select convert(varchar(100), BlogGUID) as blogguid from gixprospectos where idprospecto = {}".format(nuevo_id)
				for x in DBSession.execute(sql):
					blogguid = x.blogguid
				sql = """
				insert into Blogs (BlogGUID, FechaCaptura, UsuarioCaptura,
					...
					values ('%s', getdate(), '{}', '{}', '{}', '{}')
				""".format(blogguid, usuario, "Alta del Prospecto", "", "")
...
class ClienteRest( EAuth, QueryAndErrors ):
	...
	def store( self, record , id = None):
		...
		try:
			...
			and isnull(cuenta,0) = 0)""".format(cliente)
...
class AsignacionRest( EAuth, QueryAndErrors ):
	...
	def store( self, record , id = None):
		...
		if not candadoprecio:
			...
			found = False
			sql = """select id from gixpreciosetapa
		 	where  precio = {} and fk_etapa = {}""".format(precio, etapa)
			sql = psql(sql)
			queries.append(sql)
			precio_id = 0
			for x in ses.execute(sql):
		...
		""".format(fechadeventa, contrato, cuenta)
	...
	def store( self, record , id = None):
		...
		if True:
			...
			insert into documento (codigo, fechadeelaboracion, fechadevencimiento, fechadevencimientovar,
						...
						values ({}, '{}', '{}', '{}', {}, {}, 0, {}, {}, '')
						""".format(codigodocumento, fecha, fecha, fecha, cantidad, cantidad, tipo,
						       cuenta)
			sql = psql(sql)
			#queries.append(sql)
			...
			insert into movimiento (codigo, cantidad, fecha, relaciondepago, cargoabono,
							...
							values ({}, {}, '{}', '1/1', 'C', {}, {})
							""".format(codigomovimiento, cantidad, fecha, tipo, codigodocumento)
			sql = psql(sql)
			#queries.append(sql)
			...
			sql = """ update cuenta set saldo = saldo + {} where codigo = {} """.format(cantidad, cuenta)
			sql = psql(sql)
			#queries.append(sql)
...
class OfertaDeCompra( EAuth, QueryAndErrors ):
	...
	def store( self, record , id = None):
		...
		if not candadoprecio:
			...
			found = False
			count = 0;
			sqlcount = """select count(*) as cuantos from gixpreciosetapa where  precio={} and fk_etapa={}""".format(precio, etapa)
			for x in ses.execute(sqlcount):
				count = x.cuantos
			if count > 1:
				sqlprecio = """select idpreciocatalogo from inmueble where codigo = {}""".format(inmueble)
		...
		if localerror:
			...
			return self.edata_error( error)
		localerror = ""
		color("vendedor {}, es_subvendedor {}, vendedorvirtual {}, desactivado {}".format(p_vendedor,es_subvendedor,vendedorvirtual, desactivado))
		...
		conteo = 0
		for x in ses.execute( sql ):
			color("comision: vendedor {}, porcentaje {} ".format(x.vendedor, x.porcentaje))
			conteo = conteo + 1
		if conteo < 2:
		...
		if not ok:
			...
			return self.edata_error( error )
		found = False
		sql = "update referencias_rap set cuenta = {} where referencia = '{}'".format(cuenta, "{:010d}".format(int(referenciarap)))
		queries.append(sql)
		ok, error = self.commit(sql)
		if not ok:
			self.request.response.status = 400
		...
		insert into gixpreciosetapaofertaasignacion
								...
								""".format(etapa, contrato, fechadeventa, gixprecioetapaid,
								       precio, cuenta, fechadeventa, inmueble, precio_id, precio, inmueble)
		sql = psql(sql)
		queries.append(sql)
		ok, error = self.commit(sql)
		if not ok:
		...
		if resto:
			...
			if descuento_precio > 0:
				...
				""".format(desarrollo, desarrollo, self.usuariorecibo)
				sql = psql(sql)
				queries.append(sql)
				ok, error = self.commit(sql)
				if not ok:
				...
				values ({}, '{}', {}, {}, {}, '{}', 'A', {}, {})
				""".format(codigorecibo, fechadeventa, float(descuento_precio), 0, float(descuento_precio), "NINGUNA", desarrollo, consdesarrollo)
				...
				""".format(codigomovimiento, descuento_precio, fechadeventa, codigorecibo, fechadeventa, codigodocumento)
				sql = psql(sql)
				queries.append(sql)
				ok, error = self.commit(sql)
				if not ok:
				...
				sql = """
				update documento set saldo = saldo - {} where codigo = {}""".format(descuento_precio, codigodocumento)
				ql = psql(sql)
				queries.append(sql)
				ok, error = self.commit(sql)
				if not ok:
				...
				sql = """
				update cuenta set saldo = saldo - {} where codigo = {}""".format(descuento_precio, cuenta)
				ql = psql(sql)
				queries.append(sql)
				ok, error = self.commit(sql)
				if not ok:
		...
		if asignar:
			...
			""".format(precalificacion, avaluo, subsidio, pagare, cuenta)
		sql = psql(sql)
		queries.append(sql)
		print(paint.blue(sql))
		ok, error = self.commit(sql)
		...
		if asignar:
			...
			""".format(precalificacion, contrato, cuenta)
		...
		args = dict( codigo = cuenta, inmueble = inmueble, idprecio = precio_id, precio= precio, escuenta = True)
...
def arcadiacuadro(enganche=False):
	...
	for x in range(2003, int(tiempo.date.today().year)+1):
        ...
        #total=0
		print("ano ",x)
		sql="select i.codigo, c.fecha from inmueble i join cuenta c on c.fk_inmueble=i.codigo where datepart(yyyy, c.fecha) >={} and datepart(yyyy, c.fecha)<{} group by c.fecha, i.codigo".format(x, x+1)
		if enganche:
			sql="select fechadeelaboracion as fecha from documento where fk_tipo=1 and fk_cuenta in (select c.codigo from cuenta c join inmueble z on c.fk_inmueble=z.codigo where z.codigo in (select i.codigo from inmueble i join cuenta c on c.fk_inmueble=i.codigo where datepart(yyyy, c.fecha) >={} and datepart(yyyy, c.fecha)<{}))".format(x, x+1)
		r_year= dict(year=x)
		for x in range(1,13):
			r_year[meses[x]]=0
		r_year["total"]=0
...
@resource( collection_path='api/ventascuadroarcadias', path='api/ventascuadroarcadias/{id}')
class VentasCuadroArcadia( EAuth ):
	...
	def collection_get(self):
		p = self.request.params
		eng = p.get("enganche", "")
		return dict( meta = dict( maxyear = today(False).year ), ventascuadroarcadias = arcadiacuadro(eng) )

@resource( collection_path='api/ventascuadroarcadiasss', path='api/ventascuadroarcadiasss/{id}')
class VentasCuadroArcadias( EAuth ):
	...
	def collection_get(self):
		...
		return dict( meta = dict( maxyear = today(False).year ), ventascuadroarcadias = [dict(mes="Ene", a2003=10, a2010=30), dict(mes="Feb", a2004=5)] )
...
class MenuZen( EAuth ):
	...
	def collection_get(self):
		...
		table = rdb.db("iclar").table("menuitems")
		return dict( menus = [ dict( id = i, item = x.get("item"), 
			title = x.get("title"), intro = x.get("intro"), consulta = x.get("consulta"), reciente = self.reciente(x.get("fecha",""))) 
...
class MenuPerfil( EAuth ):
	...
	def collection_get(self):
		...
		return dict( menuperfils = [ dict( id = i, item = x ) 
			for i,x in enumerate(menuitems)] ) 
...
class UsuarioZen( EAuth ):
	...
	def put(self):
		...
		table = rdb.db("iclar").table("usuarios")
		table.filter(dict(appid = user.get("id"))).update(dict(menuitems = [x for x in menuitems.split(" ")])).run()
		return dict( zenusuario = dict( id = "1", usuario = usuario, perfil = perfil, menuitems = menuitems ))
	...
	def get(self):
		...
		return dict( zenusuario = dict( id = "1", usuario = usuario, perfil = perfil, menuitems = " ".join(menuitems) ))
...
class ProspectosBusquedas(EAuth, QueryAndErrors, OperacionesAfiliacion):
	...
	def collection_get(self):
		...
		nombrepilaprospecto = req.get(u"nombrepilaprospecto","")
		apellidomaternoprospecto = req.get(u"apellidomaternoprospecto","")
		...
		if nombreprospecto or nombrepilaprospecto or apellidomaternoprospecto:
			...
			if nombrepilaprospecto:
				where1 = u" and nombre1 like '%{}%'".format(nombrepilaprospecto)
			where2 = ""
			if nombreprospecto:
				where2 = u" and apellidopaterno1 like '%{}%'".format(nombreprospecto)
			where3 = ""
			if apellidomaternoprospecto:
				where3 = u" and apellidomaterno1 like '%{}%'".format(apellidomaternoprospecto)
			sql = u"""
		...
		if tipocuenta:
			if tipocuenta == "infonavit":
				query = query.filter(clase.contado == False).filter(clase.hipotecaria == False).filter(clase.fovisste == False).filter(clase.pensiones == False)
	...
	def collection_get(self):
		if True:
			...
			for i,x in enumerate(DBSession.execute(preparaQuery(sql)),1):
				#print "gixanip", x
				try:
					if usuariosvendedores.filter(dict(usuario = x.usuario)).count().run() > 0:
						continue
					resultado.append(
						dict(id = i,
						vendedor = x.vendedor,
		...
		if True:
			...
			try:
				sql = """
					...
					where i.fk_etapa = {} {}
					order by 2,3
				""".format(etapa, additionalWhere)
				lista = []
				for x in DBSession.execute(preparaQuery(sql)):
					vendido = x.cuenta > 0
					d = dict( id = x.inmueble, manzana = dec_enc(x.manzana, True), lote = dec_enc(x.lote, True), precio = formato_comas.format(x.precio), habilitado = x.habilitado, candado = x.candado, vendido = vendido )
					lista.append(d)
	...
	def collection_get(self):
		...
		if inmueble and prospecto:
			...
			if desarrollo:
				...
				and fk_desarrollo = {}""".format( prospecto, desarrollo )
				for x in DBSession.execute(preparaQuery(sql)):
		...
		if prospecto:
			...
			if revinculacion:
				...
				if perfil == "gerente":
					additionalWhere = "{} and p.idgerente = {}".format(additionalWhere,gerente)
				else:
					if perfil not in ("admin", "direccioncomercial", "subdireccioncomercial"):
		...
		try:
			for x in DBSession.execute( sql ):
				...
				d = dict( id = x.prospecto, nombre = x.nombre.decode("iso-8859-1").encode("utf-8"), afiliacion = x.afiliacion, \
					...
					tieneComision = tieneComision, curp = x.curp)
				resul.append(d)
...
@resource( collection_path='api/referenciasrapcuentas' , path = 'api/referenciasrapcuentas/{id}')
...
@resource( collection_path='api/inmuebleindividuals' , path = 'api/inmuebleindividuals/{id}')
class inmuebleindividual(EAuth):
	...
	def get(self):
		...
		if hay:
			return dict(inmuebleindividual=dict(id=cual, domicilio=domicilio, candadoPrecio = candadoPrecio, precioCatalogo = precioCatalogo))
		else:
			return dict()
...
class EtapasOfertas(EAuth):
	...
	def get(self):
		...
		for x in DBSession.execute(sql):
			 ...
			 nombre = "{} - {}".format(x.desarrollo.decode("iso-8859-1").encode("utf-8"), x.etapa.decode("iso-8859-1").encode("utf-8"))
		if id:
			print("si regreso algo")
			return dict(etapasofertas=dict ( id = id, departamento=departamento, nombre = nombre))
		else:
...
class EtapasTramites(EAuth):
	...
	def get(self):
		...
		if id:
			print("si regreso algo")
			return dict(etapastramite=dict ( id = id, departamento=departamento, nombre = nombre))
		else:
			print("no regreso nada")
			return dict()
...
class InmueblesDisponibles(EAuth):
	...
	def collection_get(self):
		...
		where c.fk_inmueble is null and i.fk_etapa > 39 {} and habilitado <> 0 order by i.iden2,i.iden1""".format( additionalWhere)
	...
	def collection_get(self):
		...
		try:
			...
			sql = """
				...
				""".format(top, additionalWhere, additionalWhere2)
			
			return dict(ofertasrecientes = [dict(id = i, etapa = x.etapa, oferta = x.oferta, cliente = x.cliente, nombrecliente = x.nombrecliente.decode("iso-8859-1").encode("utf-8"), cuenta = x.cuenta, manzana = dec_enc(x.manzana, True), lote = dec_enc(x.lote,True), saldo = x.saldo, fecha = x.fecha.isoformat()) for i,x in enumerate(DBSession.execute(sql),1)])
	...
	def collection_get(self):
		...
		try:
			...
			for x in DBSession.execute( preparaQuery( sql.format(select_list))):
				query_list.append(x)
			return dict(
				meta = dict( rdb = rdb, page = page, pages= pages, rowcount= rows, rowcountformatted= "{:,}".format(rows) ),
				ofertabusquedas = [dict(id = i, 
	    		oferta = x.oferta, 
	    		...
	    		telefono = x.telefono, 
	    		nombre_gerente=dec_enc(x.nombre_gerente) if x.nombre_gerente else "",
	    		nombre_subvendedor = dec_enc(x.nombre_subvendedor) ) for i,x in enumerate(query_list[left_slice : right_slice],1)])
...
@resource(collection_path='api/analisiscarteraarcadias', path='api/analisiscarteraarcadias/{id}')
...
@resource(collection_path='api/resumencobranzaarcadias', path='api/resumencobranzaarcadias/{id}')
...
@resource(collection_path='api/carteravencidaarcadias', path='api/carteravencidaarcadias/{id}')
class CarteraVencidaArcadiaRest( QueryAndErrors, EAuth):
	...
	def collection_get(self):
        ...
        order by i.fk_etapa, i.iden2, i.iden1""".format(etapas)
...
@resource(collection_path='api/lotesindividualesarcadias', path='api/lotesindividualesarcadias/{id}')
...
class LotesIndividualesArcadiaRest( QueryAndErrors, EAuth):
	...
	def store( self, record , id = None):
		...
		set preciopormetro = {}
		where codigo = {}
		""".format(record.get("preciopormetro"), id)
		print(sql)
		ok, error = self.commit(sql)
		#ok = True
		if not ok:
	...
	def collection_get(self):
		if True:
			...
			""".format(additionalWhere, additionalWhere2)
			print(sql)
			resultado = []
	...
	def collection_get(self):
		if True:
			...
			for i,x in enumerate(DBSession2.execute(preparaQuery(sql)),1):
				...
				resultado.append(dict(id = i, etapa = str(etapas.get(x.etapa,0)), cuantos = x.cuantos, valuacion = x.valuacion))
			resultado.append(dict(id = len(resultado) + 1, etapa = "Total", cuantos = tcuantos, valuacion = tvaluacion))
			DBSession2.close()
...
class InmuebleFiltradoRest(EAuth):
	...
	def collection_get(self):
		...
		try:
			...
			lista = []
			for x in DBSession.execute(sql):
				lista.append(dict(id=x.inmueble, etapa=etapa, 
					manzana=dec_enc( manzana,True),
					lote=dec_enc(x.lote, True), tramite=tramite)) 
	...
	def collection_get(self):
		...
		try:
			...
			for x in sess.execute(sql):
				...
				fechaVenta = ""
				if x.fecha:
					fechaVenta = "{:04d}/{:02d}/{:02d}".format(x.fecha.year, x.fecha.month, x.fecha.day)
			...
			sql = """
			select nombre from cliente where codigo = {}""".format(clienteId)
			for x in sess.execute(sql):
				nombreCliente = x.nombre.decode("iso-8859-1").encode("utf-8")
			d = dict(id = 1, cuenta = cuenta, saldo = saldo, clienteId = clienteId, 
				nombreCliente = nombreCliente, fechaVenta = fechaVenta, inmueble = inmueble or 0)
...
class CatalogoPreciosRest(EAuth, QueryAndErrors):
	...
	def get(self):
		...
		try:
			...
			for x in DBSession.execute(sql):
				if x.cuantos:
					vendido = True
			return dict(catalogoprecio = dict( id = cual, idPrecioCatalogo = idPrecioCatalogo, precioCatalogo = precioCatalogo,
				 candadoPrecio = candadoPrecio, vendido = vendido, habilitarInmueble = habilitado))
		except:
			print_exc()
	...
	def put(self):
		...
		try:
			...
			assert user.get("perfil", "") in ("admin","recursosfinancieros","auxiliarsubdireccion"), "perfil inadecuado"
			...
			if True:
				...
				""".format(idPrecioCatalogo,precioCatalogo, candado, habilitado, id)
			...
			try:
				...
				pass
			return dict(catalogoprecio = dict(id=id, idPrecioCatalogo = idPrecioCatalogo, precioCatalogo = precioCatalogo,
				 candadoPrecio = candadoPrecio, vendido = vendido, habilitarInmueble = habilitarInmueble))
...
@resource(collection_path='api/catalogotramiteporagregars', path='api/catalogotramiteporagregars/{id}')
class CatalogoTramitesPorAgregarRest(EAuth):
	...
	def collection_get(self):
		...
		try:
			...
			lista = []
			for x in DBSession.execute(sql):
				lista.append(dict(id=x.codigo, origen = x.origen, 
...
class TramiteGestionRest(EAuth, QueryAndErrors):
	...
	def collection_get(self):
		...
		try:
			...
			if tramites:
				if tipo == "C":
					additional_where = " and t.fk_tramite in ({})".format(tramites.replace(" ",","))
				else:
					additional_where = " and f.requisito in ({})".format( tramites.replace(" ", ","))
			...
			from inmueble i where i.fk_etapa = {}""".format(etapa)
			rows = 0
			for i,x in enumerate(DBSession.execute(preparaQuery(sql)),1):
				for tr in tramites.split(" "):
					id = rows + 1
					resultado.append(dict(id = id, inmueble = x.inmueble, manzana = dec_enc( x.manzana), lote = dec_enc(x.lote), fecha = "", tramite = int(tr), descripcion = dec_enc(dicTramites[int(tr)]) )) 
			...
			if tipo == 'G':
				...
				""".format(pseudo_table, etapa, additional_where, additional_where2 )
			else:
				sql = """
				...
				join inmueble i on t.fk_inmueble = i.codigo
				where i.fk_etapa = {} {} {}
				order by i.iden2, i.iden1, t.fk_tramite """.format(etapa, additional_where, additional_where2)
			sql=sql.replace("\t"," ")
			sql=sql.replace("\n"," ")
			lista = []
			for i,x in enumerate(DBSession.execute(sql),1):
				...
				if not x.fecha is None:
					fecha = "{:04d}/{:02d}/{:02d}".format(x.fecha.year, x.fecha.month, x.fecha.day)
				if fecha:
					resultado[dicKeys["{:08d}{:04d}".format(x.inmueble, x.tramite)]]["fecha"] = fecha
			if criterio == "F":
...
class TramitesPorCasaRest(EAuth, QueryAndErrors):
	...
	def get(self):
		...
		try:
			...
			""".format(inmueble, tramite, inmueble, tramite)
			sql=sql.replace("\t"," ")
			sql=sql.replace("\n"," ")
			lista = []
			for x in DBSession.execute(sql):
				lista.append(dict(id=cual, tramite=x.tramite, inmueble= x.inmueble,
					 montoCredito=x.montoCredito, montoSubsidio=x.montoSubsidio, 
					numeroCredito=x.numeroCredito, fechaInicial=self.conv_fecha(x.fechaInicial),
					...
					fechaVencimiento=self.conv_fecha(x.fechaVencimiento),
					 comentario=self.varchar(x.comentario), descripcion=dec_enc(x.descripcion), origen = x.origen))
	...
	def collection_get(self):
		...
		try:
			...
			join tramites c on f.requisito = c.codigo
			where m.inmueble = {}
			 order by 2""".format(inmueble, inmueble)
			sql=sql.replace("\t"," ")
			sql=sql.replace("\n"," ")
			lista = []
			for x in DBSession.execute(sql):
				id = "{:04d}{:08d}".format(int(x.tramite), int(x.inmueble))
				lista.append(dict(id=id, tramite=x.tramite, inmueble= x.inmueble,
					 montoCredito=x.montoCredito, montoSubsidio=x.montoSubsidio, 
					numeroCredito=x.numeroCredito, fechaInicial=self.conv_fecha(x.fechaInicial),
					fechaInicio=self.conv_fecha( x.fechaInicio),
					 fechaRealEntrega=self.conv_fecha(x.fechaRealEntrega),
	...
	def collection_post(self):
		...
		try:
			...
			sql = "select codigo from incorporacion_maestro where inmueble = {}".format(inmueble)
			integracion = ""
			for x in DBSession.execute(sql):
				integracion = x.codigo
			assert integracion , "No hubo registro en incorporacion_maestro para el inmueble"
			if True:
		...
		try:
			...
			if origen == "c":
				...
				if fechaInicial:
					...
					fecha = "NULL" 
				sql = "select coalesce(candado_alta,0) as candado_alta, coalesce(candado_baja,0) as candado_baja from tramites_ventas where codigo = {}".format(tramite)
				candado_alta = 0
				candado_baja = 0
				for x in ses.execute(sql):
				...
				if candado_alta > 0:
					if candado_alta < 100:
						...
						sql = """select count(*) as cuantos from tramites_ventas_movimientos 
							where fk_inmueble = {} and fk_tramite = {} and fecha is not null""".format(inmueble, candado_alta)
						cuantos = 0
						for x in ses.execute(preparaQuery(sql)):
							cuantos = x.cuantos
						assert cuantos, "Falta el tramite {}".format(candado_alta)
				if candado_baja > 0:
					sql = """select count(*) as cuantos from tramites_ventas_movimientos 
							where fk_inmueble = {} and fk_tramite = {} and fecha is not null""".format(inmueble, candado_baja)
					cuantos = 0
					for x in ses.execute(preparaQuery(sql)):
						cuantos = x.cuantos
					assert cuantos == 0, "No puede quitarse fecha pues existe el tramite {}".format(candado_baja)
				...
				""".format(fecha, self.isnull( montoCredito), self.isnull(numeroCredito), self.isnull(montoSubsidio), comentario, tramite, inmueble)
				print(sql)
				...
				where m.inmueble = {} and f.requisito = {}
				""".format(self.fecha_sql(fechaInicio),
			...
			if descripcion =='PORCENTAJE':
				...
				inmueble where codigo={}""".format(inmueble)
				etapa = 0
				manzana =0
				lote = 0
				for x in DBSession.execute(preparaQuery(sql)):
					...
					manzana = x.manzana
					lote = x.lote
					sql ="""select count(*) as cuantos from gcmex_porcentajes where lote={} """.format(lote)
					cuantos = 0
				for x in DBSession.execute(preparaQuery(sql)):
					cuantos = x.cuantos
					if cuantos:
						sql="""select fk_etapa as etapa, iden2 as manzana, iden1 as lote from inmueble where codigo={}""".format(inmueble)
						etapa = 0
						manzana =0
						lote = 0
						for x in DBSession.execute(preparaQuery(sql)):
							...
							sql="""update gcmex_porcentajes set porcentaje = {} where lote={}""".format(porcentaje, lote)
							sql = self.cleanSql(sql)
							sql = sql.encode("iso-8859-1")
							cu.execute(sql)
						cn_sql.commit()
						...
						for x in DBSession.execute(preparaQuery(sql)):
							...
							if numeroCredito != "":
								porcentaje= float(numeroCredito)
								sql="""insert into gcmex_porcentajes (fk_etapa, manzana, lote, porcentaje) values ({},{},{},{})""".format(etapa, manzana, lote, porcentaje)
								sql = self.cleanSql(sql)
								sql = sql.encode("iso-8859-1")
								cu.execute(sql)
					cn_sql.commit()
			try:
				...
				pass
			return dict(tramite = dict(id=cual, tramite=tramite, inmueble= inmueble,
			montoCredito=montoCredito, montoSubsidio=montoSubsidio, 
					numeroCredito=numeroCredito, fechaInicial=fechaInicial,
					fechaInicio=fechaInicio,
					 fechaRealEntrega=fechaRealEntrega,
					 fechaEstEntrega=fechaEstEntrega,
					fechaVencimiento=fechaVencimiento,
					 comentario=comentario.encode("utf-8"), descripcion=descripcion, origen = origen))
		except AssertionError as e:
			print_exc()
			self.request.response.status = 400
			error = e.args[0]
	...
	def deleterecord(self, id, token):
		...
		try:
			cual = self.request.matchdict['id']
			assert user.get("perfil", "") in ("admin","comercial","subdireccioncomercial", "subdireccion", "auxiliarsubdireccion", "gestionurbana"), "perfil inadecuado"
			assert usuario, "no hay usuario"
			tramite = int(cual[:4])
			inmueble = int(cual[4:])
			assert tramite, "no hay tramite"
    ...
    try:
        ...
        if pdf:
            ...
            if tipo in ("caracteristicas oferta".split(" ")):
        	    ...
        	    datos = eval(func_name)(etapa, oferta, precalificacion, avaluo, subsidio, pagare)
        ...
        if printer == "email":
            ...
            if xls:
        	    ...
        	    table.insert(dict(filepath = excel_name, user = user , email = email, timestamp = ts)).run()
...
def useremail(request):
	try:
		...
		rdb.db("printing").table("useremail").filter(rdb.row["user"] == usuario).delete().run()
		
		if email:
			rdb.db("printing").table("useremail").insert(dict(user = usuario, email = email) ).run()
...
def lotespinares(request):
	...
	from inmueble i left join cuenta c
	on  i.codigo = c.fk_inmueble
	where i.fk_etapa in ({})""".format(etapa)
	resultado = []
	for x in DBSession2.execute(preparaQuery(sql)):
		manzana= dec_enc(x.manzana)
		idval = "{}{}".format(manzana, x.lote)
		resultado.append(dict(id = idval, manzana=manzana.strip(), lote=x.lote.strip(),  etapa = x.etapa, estatus=x.estatus, preciopormetro=x.preciopormetro))
	DBSession2.close()
...
def routeauth(request):
	...
	if valid == "1":
		...
		try:
			...
			rdb.db("iclar").table(tname1).insert(dict( usuario = usuario.lower(),  route = route, timestamp = ts, token=token )).run()
			...
			for row in rdb.db("iclar").table(tname2).filter(dict(route = "application" )).run():
				f = row.get("feature", "")
                #isInFeature = False
				value = False
				if f:
			...
			color( req, 'c' )
			c = redis.Redis(host = crs.get("redis.host"), port = crs.get("redis.port"), db = 0 )
...
def revoke( request ):
	...
	try:
		if token:
			...
			rdb.db("iclar").table("usuarios").filter(dict(usuario=usuario.upper())).update(dict(isTwoFactorAuthenticated = False)).run()
...
def get_google_token( code, refresh = False, source = None ):
    ...
    try:	
        ...
        ts = rdb.expr( datetime.now( rdb.make_timezone('00:00') ))
        db.table("google_token").insert(dict(token = access_token, active = True, timestamp = ts )).run()
        if refresh_token:
...
def general_token( request, version = 1 ):
	...
	gerente = 0
	try:
		if zen_profile in "admin direccioncomercial subdireccioncomercial comercial presidencia auxiliarpresidencia subdireccion cobranza auxiliarsubdireccion finanzas direccion auxiliardireccion especialcomercial recepcion recursoshumanos gestionurbana direccioncontable subdireccioncontable gestioncrediticia mercadotecnia" :
			vendedor = 0
	...
	cr.dicTokenUser[auth_token] = dict( id = id_user, gravatar = gravatar, routes = routes, menuitems = menuitems, gerente = gerente, vendedor = vendedor, usuario = email, perfil = zen_profile )
   

	try:
		ts = rdb.expr( datetime.now( rdb.make_timezone('00:00') ))
		rdb.db("iclar").table(tname).insert(dict( usuario = email.upper(), token = auth_token, created = datetime.now().isoformat(), timestamp = ts, active = True, dic_token_user = cr.dicTokenUser[auth_token])).run()
	except:
		print_exc()
	try:	
