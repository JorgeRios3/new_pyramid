...
def nbb_one_way_process_request(request = "dummy", source = "test", user = "test", arguments = None):
	k = r.incr("nbb_key")
	nk = "P{:010d}".format(k)
	d = dict(request = request, source = source, user = user)
	if arguments and isinstance( arguments, dict):
...
def venta_por_vendedor_arcadia_terrenos():
	print("here nbb")
	print(process_request( request = "venta_por_vendedor_arcadia_terrenos", source = "zen", user = "zen", arguments = dict(vendedor = "0", con_enganche = False)))

def venta_por_vendedor_arcadia(vendedor = 0, con_enganche = False):
	return json.loads(process_request( request = "venta_por_vendedor_arcadia", source = "zen", user = "zen", arguments = dict( vendedor = vendedor, con_enganche = con_enganche)))

def resumen_cobranza_am(hardCall = True):
	RESUMEN_FILE = "rescobarcadia.p"
	if hardCall:
		result = process_request( request = "resumen_cobranza_am", source = "bar", user = "foo"  )
...
def lotes_pagados_arcadia():
	r = json.loads(process_request(request = "lotes_pagados_escriturados_arcadia", source = "zen", user= "zen"))
	#print r
	return r
