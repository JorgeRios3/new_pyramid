...
from sqlalchemy.orm import scoped_session, sessionmaker, joinedload, relationship
...
class Mfa_Device(Base):
    ...
    def __repr__(self):
        try:
            return "Mfa_Device ( instance ) : id={} id_app_user={} iam_username={} description={} arn={} access_key={} secret_key={}".format(
                self.id,
                self.id_app_user,
                self.iam_username,
                self.description,
...
class Mfa_User(Base):
    ...
    devices = relationship(
        Mfa_Device,
        primaryjoin=id == Mfa_Device.id_app_user,
        order_by=Mfa_Device.id,
        backref="myuser",
    )

    def __repr__(self):
        try:
            return "Mfa_User (instance) : id={} user={} active={} number_of_devices={}".format(
                self.id, self.user, self.active, len(self.devices)
            )
        except:
...
class CuentaPagare(Base):
    ...
    def saldovencido(self):
        dias = 30
        q = (
            DBSession.query(func.isnull(func.sum(DocumentoPagare.saldo), 0))
            .filter(DocumentoPagare.fk_cuenta == self.codigo)
            .filter(
                func.datediff(
                    literal_column("d"), DocumentoPagare.fechven, func.getdate()
                )
                >= dias
            )
        )
    ...
    def all_alterno(cls):
        try:
            q = (
                DBSession.query(
                    CuentaPagare,
                    func.isnull(func.sum(DocumentoPagare.saldo), 0).alias(
                        "saldovencido2"
                    ),
                )
                .join(DocumentoPagare)
                .options(joinedload(CuentaPagare.codigo))
                .filter(func.datediff(literal_column("d")))
                .group_by(CuentaPagare.codigo)
            )
        except:
    ...
    def cornice_json(self):
        ...
        if self.saldo == self.lacuenta.saldo:
            if self.saldoformateado == self.documento17.saldoformateado:
                warning = ""
        q = (
            DBSession.query(DocumentoPagare)
            .filter(DocumentoPagare.fk_cuenta == self.codigo)
            .order_by(DocumentoPagare.codigo)
        )
        ...
        if True:
            q = (
                DBSession.query(Movimiento)
                .filter(Movimiento.fk_documento == self.documento)
                .filter(Movimiento.cargoabono == "A")
                .order_by(Movimiento.codigo)
            )
        ...
        d = dict(
            ...
            cuenta=self.cuenta,
            saldo=self.saldo,
            saldoformateado=self.saldoformateado,
            clavecliente=self.lacuenta.cliente.codigo,
            nombrecliente=self.lacuenta.cliente.nombre.decode("ISO-8859-1").encode(
                "UTF-8"
            ),
            fechacuenta=self.lacuenta.fecha.strftime("%Y-%m-%d"),
            saldocuenta=self.lacuenta.saldo,
            saldocuentaformateado="{:,.2f}".format(self.lacuenta.saldo),
            casa="{} {}".format(
                self.lacuenta.inmueble.iden2, self.lacuenta.inmueble.iden1
            ),
            pagares=DocumentoPagare.cuantos_pagares(self.codigo),
            pagaresconsaldo=DocumentoPagare.cuantos_pagares(self.codigo, True),
            saldodocto17formateado=self.documento17.saldoformateado,
            warning=warning,
            telefonocasa=self.lacuenta.cliente.telefonocasa or "",
            telefonotrabajo=self.lacuenta.cliente.telefonotrabajo or "",
            email=self.lacuenta.cliente.email or "",
            documentos=documentos,
            saldovencido=saldovencido,
            abonosdocumento17=abonosdocumento17,
        )
...
class DocumentoPagare(Base):
    ...
    def diasvencidos(self):
        dias = (
            ...
            .filter(DocumentoPagare.codigo == self.codigo)
            .one()[0]
        )
        return dias
    ...
    def vencido(self):
        try:
            ...
            dias = (
                ...
                .filter(DocumentoPagare.codigo == self.codigo)
                .filter(DocumentoPagare.saldo > 0)
                .one()[0]
            )
            if dias < 0:
                dias = 0
    ...
    def cornice_json(self):
        ...
        d = dict(
            ...
            abono=formato_comas.format(self.abono),
            vencido=vencido,
        )
        return d
    ...
    def vencido_a(cls, dias):
        try:
            q = DBSession.query(func.sum(cls.saldo)).filter(
                func.datediff(literal_column("d"), cls.fechven, func.getdate()) >= dias
            )
        except:
            traceback.print_exc()
...
class Cuenta(Base):
    ...
    def consaldo(cls, saldo=0):
        return (
            session.query(func.count(cls.saldo))
            .filter(cls.saldo > saldo)
            .filter(cls.codigo > 14672)
            .one()[0]
        )
    ...
    def sumasaldos(cls, cualquiera=True, sinasignar=False):
        ...
        q = session.query(func.isnull(func.sum(cls.saldo), 0)).filter(
            cls.codigo > 14672
        )
        if not cualquiera:
            if sinasignar:
...
class GerentesVentas(Base):
    ...
    def cornice_json(self):
        return dict(
            ...
            nombre=self.nombre,
            activo=self.activo,
            prospectador=self.prospectador,
        )
...
class Prospecto(Base):
    ...
    def existeAfiliacion(cls, afiliacion):
        try:
            query = session.query(Prospecto).filter(
                Prospecto.afiliacionimss == afiliacion
            )
    ...
    def cornice_json(self):
        d = dict(
            id=self.id,
            apellidopaterno=d_e(self.apellidopaterno1),
...
CuentaPagare.lacuenta = relationship(
    "Cuenta",
    primaryjoin=CuentaPagare.fk_cuenta == Cuenta.codigo,
    foreign_keys=CuentaPagare.fk_cuenta,
    remote_side=Cuenta.codigo,
    backref="mi_cuenta_pagare",
    viewonly=True,
)

Cuenta.cliente = relationship(
    "Cliente",
    primaryjoin=Cuenta.fk_cliente == Cliente.codigo,
    foreign_keys=Cuenta.fk_cliente,
    remote_side=Cliente.codigo,
    backref="mi_cuenta",
    viewonly=True,
)
Cuenta.inmueble = relationship(
    "Inmueble",
    primaryjoin=Cuenta.fk_inmueble == Inmueble.codigo,
    foreign_keys=Cuenta.fk_inmueble,
    remote_side=Inmueble.codigo,
    backref="mi_cuenta_c",
    viewonly=True,
)

CuentaPagare.documento17 = relationship(
    "Documento",
    primaryjoin=CuentaPagare.fk_documento == Documento.codigo,
    foreign_keys=CuentaPagare.fk_documento,
    remote_side=Documento.codigo,
    backref="mi_cuenta_alterna",
    viewonly=True,
)

CuentaPagare.documentos = relationship(
    "DocumentoPagare",
    primaryjoin=CuentaPagare.codigo == DocumentoPagare.fk_cuenta,
    foreign_keys=DocumentoPagare.fk_cuenta,
    remote_side=CuentaPagare.codigo,
    backref="mi_cuenta",
    viewonly=True,
)
