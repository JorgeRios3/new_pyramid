...
def nbb_process_request(
    func="dummy", source="test", user="test", arguments=None, msgpack=False
):
...
def nbb_one_way_process_request(
    func="dummy", source="test", user="test", arguments=None
):
    k = r.incr("{}-nbb-key".format(PREFIX))
    nk = "{}-P{:010d}".format(PREFIX, k)
    d = dict(func=func, source=source, user=user)
...
def mssqltableCount(source="bar"):
    p = json.loads(
        process_request(
            func="mssqltablecount",
            source=source,
            user="foo",
            arguments=dict(table="cuenta"),
        )
    )
    return p
